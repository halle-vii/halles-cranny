...
export function Navbar({ activeSection }: NavbarProps) {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 block px-3 pt-4 sm:px-4 sm:pt-6">
      <nav
        aria-label="Primary"
        className="relative mx-auto flex h-[55px] w-full max-w-[610px] items-center justify-between rounded-[50px] bg-eggshell p-1 shadow-[0_8px_24px_rgba(0,0,0,0.08)] sm:h-[67px] sm:px-2"
      >
        {navItems.map((item) => {
